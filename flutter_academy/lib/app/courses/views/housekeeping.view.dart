...
import 'package:flutter_academy/app/courses/view_models/lists/housekeeping_list.vm.dart';
import 'package:flutter_academy/infrastructure/courses/model/room.model.dart';
import 'package:flutter_academy/infrastructure/courses/model/housekeeping.model.dart';
    ...
    return roomsAsync.when(
      data: (rooms) {
        ...
        final unknownRooms = rooms
            .where((r) => !statusPriority.contains(r.cleaningStatusId ?? 3))
            .toList();
...
class _InteractiveRoomRow extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final statusId = room.cleaningStatusId ?? 3; // Default to 'Clean'
    final statusName = cleaningStatusNames[statusId] ?? 'Unknown';
      ...
      child: Container(
        ...
        child: ListTile(
          title: Text('Room ${room.roomNumber}',
              style: const TextStyle(fontWeight: FontWeight.bold)),
          subtitle: Text('Current Status: $statusName'),
  ...
  void _showStatusUpdateDialog(
      BuildContext context, WidgetRef ref, Room room, int propertyId) {
    showDialog(
      context: context,
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: ListView.builder(
              shrinkWrap: true,
              itemCount: cleaningStatusNames.length,
              itemBuilder: (context, index) {
                int key = cleaningStatusNames.keys.elementAt(index);
                ...
                return ListTile(
                  ...
                  onTap: () async {
                    ...
                    final success = await roomService.updateCleaningStatus(
                        propertyId, int.parse(room.id), key);
